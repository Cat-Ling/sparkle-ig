...
- **Notify Outside the App**: Sends a system notification when Instagram is not in front, so activity that happens while you are elsewhere still reaches you. On by default. While you are in the app the in-app pill is used instead, so nothing is filed twice.
